...
![](../images/wedding_family.png){width=67% style="margin-left: auto; margin-right: auto;display: block;" .nolightbox fig-alt='logo'}
...
for (i in seq_along(photos)){
  ...
  cat(glue('![]({{ baseurl }}/{{ fs::path_file(photos[i]) }}){.lightbox style="column-span: {{ span }};" group="highlights"}', .open = '{{', .close = '}}'), '
')
